...
function format_DES_output()
{
   ...
   // copy to textbox
    document.forms["LoginForm"].outdata.value = str;
}
...
function do_des( do_encrypt )
{
   ...
   get_value( inData, document.forms["LoginForm"].indata.value,
		true );

   // problems??
   if ( inData[0] == ERROR_VAL )
   {
      document.forms["LoginForm"].details.value = accumulated_output_info;
      return;
   }
   accumulate_bitstring( "Input bits:", inData, 8 );

   // get the key from the user
   get_value( Key, document.forms["LoginForm"].rndval.value, false );
   // problems??
   if ( Key[0] == ERROR_VAL )
   {
      document.forms["LoginForm"].details.value = accumulated_output_info;
   ...
   format_DES_output( );
   document.forms["LoginForm"].details.value = accumulated_output_info;
   document.forms["LoginForm"].details.value ='';
}
...
function do_tdes( do_encrypt )
{
   ...
   get_value( KeyA, document.forms["LoginForm"].rndval.value, false );
   // problems??
   if ( KeyA[0] == ERROR_VAL )
   {
      document.forms["LoginForm"].details.value = accumulated_output_info;
   ...
   get_value( KeyB, document.forms["LoginForm"].keyb.value, false );
   // problems??
   if ( KeyB[0] == ERROR_VAL )
   {
      document.forms["LoginForm"].details.value = accumulated_output_info;
