...
if (strcmp($action,"upload")==0)
	{
		$file_cnt=0;
		$dir_name="../documents_article/".$cod_tarh_f;
		...
		if($file_cnt < 12)
		{
			$status_upload=upload_file("../documents_article",$cod_tarh_f,"");
			if(!strcmp($status_upload,"-5") == 0 and !strcmp($status_upload,"-2") == 0 )
			{
				// $status="upload_error";
				$action="ضميمه ثبت شد"."<br>".$cod_tarh_f;
...
{
	delete_file("../documents_article",$cod_tarh_f,$delete_id);
	$action="ضميمه حذف شد"."<br>".$cod_tarh_f;
	set_log($action,$admin,date("Y-m-d, g:i a"));
	$dir_name="../documents_article/".$cod_tarh_f;
 	if ($dir = @opendir($dir_name))
	{
